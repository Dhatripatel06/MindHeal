...
class _SmartwatchConnectionPageState extends State<SmartwatchConnectionPage>
  ...
  Future<void> _startScanning() async {
    ...
    try {
      ...
      FlutterBluePlus.scanResults.listen((results) {
        setState(() {
          _devices = results
              .where((r) => r.device.name.isNotEmpty && _isWearableDevice(r.device.name))
              .map((r) => r.device)
              .toList();
        });
      });
  ...
  bool _isWearableDevice(String name) {
    final wearableKeywords = [
      'Watch', 'Fit', 'Band', 'Heart', 'Health', 'Samsung', 'Apple', 
      'Fitbit', 'Garmin', 'Polar', 'Suunto', 'Xiaomi', 'Amazfit',
      'Huawei', 'Honor', 'Realme', 'OnePlus', 'OPPO', 'Vivo'
    ];
    
    return wearableKeywords.any((keyword) => 
