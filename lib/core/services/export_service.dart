...
class ExportService {
  ...
  static Future<File> exportAsJSON(List<MoodSession> sessions) async {
    ...
    final file = File('${directory.path}/mood_data_${DateTime.now().millisecondsSinceEpoch}.json');
  ...
  static Future<File> exportAsCSV(List<MoodSession> sessions) async {
    final directory = await getApplicationDocumentsDirectory();
    final file = File('${directory.path}/mood_data_${DateTime.now().millisecondsSinceEpoch}.csv');
  ...
  static Future<File> exportStatsSummary(
    ...
    final directory = await getApplicationDocumentsDirectory();
    final file = File('${directory.path}/mood_summary_${DateTime.now().millisecondsSinceEpoch}.txt');
    ...
    if (sessions.isNotEmpty) {
      ...
      content.writeln('Date Range: ${sessions.first.timestamp.toLocal().toString().split(' ')[0]} to ${sessions.last.timestamp.toLocal().toString().split(' ')[0]}');
      content.writeln('');
    }
    ...
    for (final session in sessions) {
      analysisTypes[session.analysisType] = (analysisTypes[session.analysisType] ?? 0) + 1;
    }
