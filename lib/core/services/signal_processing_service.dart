...
class SignalProcessingService {
  ...
  static const double _lowCutoff = 0.5; // 30 BPM
  static const double _highCutoff = 4.0; // 240 BPM
  ...
  BPMResult calculateBPM() {
    ...
    intervals = intervals.where((interval) => 
      interval >= 0.25 && interval <= 2.0).toList();
  ...
  List<int> _detectPeaks() {
    ...
    double mean = _movingAverage.reduce((a, b) => a + b) / _movingAverage.length;
    double variance = 0;
